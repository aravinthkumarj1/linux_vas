...
# Status of all the peripherals testing will be dumped to a text file(TestResult_log.txt)
# README - movis/README

		###### Create Test Result file ######

name=/movis/TestResult_log
if [[ -e $name0.txt ]] ; then
    i=1
...
if [ "$p_no" == "3" ] || [ "$auto" == "1" ]; then
	echo -e "\n### Audio Test started ###"			
		...
		echo -e  "\e[5m\e[93m Connect the Loopback cable And remove HeadPhone\e[0m"
                echo -e "After the connection press \e[1m ENTER \e[0m"
                read -s -n 1  key1
                arecord -d 15 /movis/sample-rec.wav & aplay /movis/sample.wav 
                export ai_test=`echo $?`
		echo -e  "\e[5m\e[93m Connect the HeadPhone And remove Loopback cable \e[0m"
                echo -e "After the connection press \e[1m ENTER \e[0m"
                read -s -n 1  key2
		aplay /movis/sample-rec.wav 
		export aio_test=`echo $?`
                if [ $ai_test -eq 0 ] || [ $aio_test -eq 0 ]; then
                        echo -e "Line-In Test \e[32m successful \e[0m"		| tee -a $name.txt
                else
                        echo -e "Line-In Test \e[31m Failure \e[0m"		| tee -a $name.txt
                fi
fi
...
if [ "$p_no" == "6" ] || [ "$auto" == "1" ]; then
		...
		read -p "Enter the USB mount point(ie:sdb1) " otg
...
if [ "$p_no" == "7" ] || [ "$auto" == "1" ]; then
        echo -e "\n### USB-HOST Test started ###"
                ...
                read -p "Enter the USB mount point(ie:sdc1) " usb
